...
class P2PExecution:
    ...
    def run(self):
        ...
        for attempt in range(self.retries + 1):
            ...
            if attempts == 1:
                tentativa_txt = "primeiro envio"
            else:
                tentativa_txt = f"REENVIO #{attempts - 1}"
        ...
        if rx_hex is not None:
            if attempts == 1:
                ...
                resumo = f"com reenvio (tentativas={attempts})"
        else:
            resumo = "SEM RESPOSTA (esgotadas as tentativas)"

        self.emit(
            f"<< Resultado TX: RX_hex={rx_hex}, rssi={rssi}, snr={snr}, "
            f"rtt={rtt_ms}, tentativas={attempts} -> {resumo}"
        )
