...
#   (Agora a recepção APENAS armazena o pacote recebido,
#    sem checar se é igual ao enviado)
# ===========================================================
class P2PExecution:
    ...
    def parse_line(line: str):
        ...
        Fazemos a busca do token 'RXP2P' e assumimos:
    ...
    def run(self):
        ...
        attempts = 0
        rx_hex = None
        # NÃO VAMOS MAIS VERIFICAR MATCH
        match = False
        rssi = snr = None
        rtt_ms = None
        ...
        for attempt in range(self.retries + 1):
            ...
            for line in self.device.read_until(self.timeout):
                ...
                if ph:
                    rx_hex = ph
                    # NÃO COMPARAMOS COM payload_hex
                    rssi = rxrssi
                    snr = rxsnr
        ...
        result = PacketResult(
            ...
            match=match,  # permanece False, apenas para manter o campo
            attempts=attempts,
            rssi=rssi,
            snr=snr,
            rtt_ms=rtt_ms,
        ...
        self.emit(f"<< Resultado TX: RX_hex={rx_hex}, rssi={rssi}, snr={snr}, rtt={rtt_ms}")
...
class App(tk.Tk):
    ...
    def parse_line(line: str):
        """
        Mesmo parser robusto usado em P2PExecution.parse_line
        """
        parts = line.split(":")
        rssi = snr = None
        payload_hex = None
    ...
    def start_rx(self):
        ...
        def rx_loop():
            ...
            try:
                ...
                # AT+NWM=0, AT+P2P=..., AT+PRECV=65534
